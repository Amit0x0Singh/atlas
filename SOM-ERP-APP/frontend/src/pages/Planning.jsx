...
function PlanningDrawer({ plan, pendingItem, onSave, onClose, equipList, employees }) {
  ...
  const isNew   = !plan // planning a new item from unplanned list
  ...
  async function submit(e) {
    ...
    setSaving(true)
    try {
      // Map new field names back to API field names
      const payload = {
        ...form,
        pp1: form.primaryPack,
        pp2: form.primaryPack2,
        sp1: form.secondaryPack,
        noOfSp: form.noOfSecPacks,
      ...
      <div className="w-full max-w-xl bg-white shadow-2xl flex flex-col h-full overflow-hidden">
        ...
        <form onSubmit={submit} className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
              Equipment &amp; Cycles
              <span className="ml-2 text-gray-400 normal-case font-normal">Total: {totalQty} {uom}</span>
            </p>
                  ...
                  <select value={form.equipment} onChange={e => onEquipChange(e.target.value)}
                    ...
                    {eqpOptions.map(e => (
                      <option key={e.equipId||e.id} value={e.equipName}>
                        {e.equipName}{e.workingVolume ? ` (cap: ${e.workingVolume} ${uom})` : ''}
                      </option>
                    ))}
                  </select>
                ) : (
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
              Product Details <span className="text-gray-300 normal-case font-normal">— pre-filled from Sales Order</span>
            </p>
            <div className="grid grid-cols-2 gap-3">
              {showCarrier && (
                <div>
              ...
              {showNano && (
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Total Quantity (L)</label>
                  <input value={totalQty} readOnly className="w-full border border-gray-200 bg-gray-50 rounded-lg px-3 py-2 text-sm text-gray-500" />
                </div>
              )}
            </div>
          </div>
          ...
          {showPacking && (
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
                Packing Details <span className="text-gray-300 normal-case font-normal">— pre-filled from Sales Order</span>
              </p>
              <div className="grid grid-cols-3 gap-3">
                <div>
          ...
          {/* ── Microbial SFG ─────────────────────────────────────────────── */}
          {plan && (
            <MicrobialSFGPanel plan={plan} multiplicationFactor={multiplicationFactor} onMfChange={setMultiplicationFactor} />
          )}
        ...
        <div className="px-5 py-4 border-t bg-gray-50 flex-shrink-0 space-y-2">
          ...
          <div className="flex gap-2">
            <button type="submit" form="planning-form" onClick={submit} disabled={saving}
            ...
            <button type="button" onClick={onClose}
              className="px-4 border border-gray-300 py-2.5 rounded-lg text-sm hover:bg-gray-100">
              Cancel
            </button>
          </div>
        </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200">
            <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 whitespace-nowrap">DI Number</th>
            ...
            <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">Quantity</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">ETD</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">RM</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">Biomass</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">SFG</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">Priority</th>
            ...
            return (
              <tr key={item.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${overdue?'bg-red-50/40':''}`}>
                <td className="px-4 py-3">
                  <span className="font-mono text-xs font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded">{so.diNo||'—'}</span>
                </td>
                <td className="px-4 py-3 text-sm font-medium text-gray-800 max-w-[120px] truncate">{so.customerName||'—'}</td>
                <td className="px-4 py-3">
                  <div className="text-sm font-semibold text-gray-900 max-w-[140px] truncate">{item.inhouseProductName||'—'}</div>
                </td>
                <td className="px-4 py-3">
                  {section !== '—' ? (
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${SECTION_BADGE[section]||'bg-gray-100 text-gray-600'}`}>
                ...
                <td className="px-4 py-3">
                  <div className={`text-xs font-semibold ${overdue?'text-red-600':daysLeft<=7?'text-orange-500':'text-gray-600'}`}>
                    {etd ? fmt(etd) : '—'}
                  </div>
                  {daysLeft !== null && (
                    <div className="text-xs text-gray-400">{overdue?`${Math.abs(daysLeft)}d late`:`${daysLeft}d left`}</div>
                  )}
                </td>
                {/* RM status — derived from priority-based logic */}
                <td className="px-4 py-3 text-center">
                  {so.priority === 'VERY_URGENT' ?
                    <span title="Check RM">🟡</span> :
                    <span title="Pending check">⚪</span>
                  }
                </td>
                {/* Biomass */}
      ...
      <table className="w-full text-sm min-w-[1200px]">
        <thead className="sticky top-0 z-10">
          <tr className="bg-gray-800 text-white">
            <th className="text-left px-3 py-3 text-xs font-semibold whitespace-nowrap">Task ID</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Planned Date</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">DI No.</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Product</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Section</th>
            <th className="text-right px-3 py-3 text-xs font-semibold">Quantity</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Equipment</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Shift</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Incharge</th>
            <th className="text-center px-3 py-3 text-xs font-semibold">RM</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Issue Status</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Production</th>
            <th className="text-center px-3 py-3 text-xs font-semibold">QC</th>
            <th className="text-left px-3 py-3 text-xs font-semibold">Task Status</th>
          </tr>
        </thead>
        <tbody>
          {plans.map((plan, idx) => {
            ...
            return (
              <tr key={plan.id}
                className={`border-b border-gray-100 hover:bg-indigo-50/30 cursor-pointer transition-colors group ${rowBg}`}
                onClick={() => onRowClick(plan)}>
                <td className="px-3 py-3">
                  <span className="font-mono text-xs font-bold text-indigo-700">{plan.planId}</span>
                </td>
                <td className="px-3 py-3 text-xs text-gray-600 whitespace-nowrap">{fmt(plan.plannedDate)}</td>
                <td className="px-3 py-3">
                  <span className="font-mono text-xs text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded">{plan.diNo||'—'}</span>
                </td>
                <td className="px-3 py-3">
                ...
                <td className="px-3 py-3">
                  {plan.sectionType ? (
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${SECTION_BADGE[plan.sectionType]||'bg-gray-100 text-gray-600'}`}>
                      {SECTION_ICON[plan.sectionType]} {plan.sectionType}
                    </span>
                  ) : '—'}
                </td>
                <td className="px-3 py-3 text-right text-sm font-bold text-gray-700 whitespace-nowrap">
                  {plan.totalQty} <span className="text-xs text-gray-400 font-normal">{plan.uom}</span>
                </td>
                ...
                <td className="px-3 py-3 text-center text-base" title={`RM: ${plan.rmCheckStatus}`}>
                  {RM_ICON[plan.rmCheckStatus] || '⚪'}
                </td>
                <td className="px-3 py-3">
                ...
                <td className="px-3 py-3">
                  <select
                    ...
                    onChange={e => { e.stopPropagation(); onStatusChange(plan.id, e.target.value) }}
                ...
                <td className="px-3 py-3 text-center">
                  {qcStatus !== '—' ? (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${QC_STATUS_STYLE[qcStatus]||'bg-gray-100 text-gray-500'}`}>
                      {qcStatus}
                    </span>
...
function DailyExecutionSchedule({ plans }) {
  ...
  const filtered = plans.filter(p => {
    ...
    const d = p.plannedDate.split('T')[0]
    return d <= scheduleDate
  })

  // Group: section → shift → equipment → tasks
  const grouped = {}
  filtered.forEach(p => {
  ...
  function handlePrint() {
    ...
    win.document.write(`<html><head><title>Daily Schedule — ${scheduleDate}</title>
      ...
      h3{font-size:11px;color:#555;margin:8px 0 4px} h4{font-size:10px;color:#777;margin:6px 0 2px}
      table{width:100%;border-collapse:collapse;margin-bottom:8px}
      th,td{border:1px solid #ddd;padding:5px 8px;text-align:left;font-size:11px}
      th{background:#eee;font-weight:bold} .badge{display:inline-block;padding:1px 6px;border-radius:9px;font-size:10px}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="mt-5">
          <button onClick={handlePrint}
            className="flex items-center gap-2 px-4 py-2 bg-gray-800 text-white text-sm font-semibold rounded-lg hover:bg-gray-900 transition">
            🖨 Print Schedule
          </button>
        </div>
        <div className="mt-5 text-xs text-gray-400">
          {filtered.length} task{filtered.length !== 1 ? 's' : ''} up to {scheduleDate}
        </div>
      </div>
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-gray-50">
                            <th className="text-left px-4 py-2 font-semibold text-gray-400">Task ID</th>
                            <th className="text-left px-4 py-2 font-semibold text-gray-400">Product</th>
                            <th className="text-right px-4 py-2 font-semibold text-gray-400">Qty</th>
                            <th className="text-left px-4 py-2 font-semibold text-gray-400">Incharge</th>
                            <th className="text-left px-4 py-2 font-semibold text-gray-400">DI No.</th>
...
export default function Planning() {
  ...
  const [drawer,         setDrawer]         = useState(null) // { plan, pendingItem }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [planRes, pendRes, dashRes] = await Promise.all([
  ...
  async function handleSave(planId, form, pendingItem) {
    // 1. Update (or create via status change) the plan
    if (planId) {
      await planningApi.updatePlan(planId, {
        plannedDate:      form.plannedDate,
        shift:            form.shift,
        batchIncharge:    form.batchIncharge,
        ...
        stageInfo:        form.stageInfo,
        noOfCycles:       form.noOfCycles,
        cycleBatchSize:   form.cycleBatchSize,
        unitPackQty:      form.unitPackQty,
        ...
        noOfSp:           form.noOfSecPacks,
        labels:           form.labels,
        packingSpec:      form.packingSpec,
        perDayCompletion: form.perDayCompletion,
        femaleWorkers:    form.femaleWorkers,
        maleWorkers:      form.maleWorkers,
        remarks:          form.remarks,
        status:           form.status || 'PLANNED',
      })

      // 2. Find the plan object to auto-generate BOMs
      const plan = plans.find(p => p.id === planId)
      if (plan) {
        // Auto-generate Formulation BOM
        try {
          await bomSendApi.create({
            planId:      plan.id,
            productCode: plan.productCode || '',
            productName: plan.productName,
            batchNo:     form.batchCode   || plan.batchCode || '',
            diNo:        plan.diNo        || '',
        ...
        // Auto-generate Packing BOM for LIQUID/POWDER/GRANULES
        ...
        // 3. Create production indent + batch if equipment set
        if (form.equipment || plan.equipment) {
          try {
            const cycles   = parseInt(form.noOfCycles)   || 1
            const cycleQty = form.cycleBatchSize || String(plan.totalQty)
            ...
            await indentApi.create({
              ...
              equipment:      form.equipment || plan.equipment || '',
          ...
          try {
            await productionApi.create({
              productCode:    plans.find(p=>p.id===planId)?.productCode || '',
              productName:    plan.productName,
              ...
              equipment:      form.equipment || plan.equipment || '',
              plannedDate:    form.plannedDate || plan.plannedDate || null,
              shift:          form.shift || plan.shift || '',
              batchIncharge:  form.batchIncharge || plan.batchIncharge || '',
              noOfCycles:     parseInt(form.noOfCycles) || 1,
              cycleBatchSize: parseFloat(form.cycleBatchSize) || 0,
              carrier:        form.carrier || plan.carrier || '',
              specs:          form.specs || plan.specs || '',
              planId:         plan.planId,
            })
          } catch { /* non-fatal */ }
        }
  ...
  // KPI filter — filter plans by clicked KPI
  ...
  return (
    <div className="max-w-full bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-30">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {runMsg && (
              <span className={`text-xs font-semibold px-3 py-1.5 rounded-lg ${runMsg.startsWith('✅')?'bg-green-50 text-green-700 border border-green-200':'bg-red-50 text-red-700 border border-red-200'}`}>
                {runMsg}
              </span>
            )}
      ...
      <div className="px-6 py-6 space-y-8">
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            SECTION 1 — FACTORY PLANNING DASHBOARD
        ═══════════════════════════════════════════════════════════════════ */}
        <section>
          <div className="flex items-center gap-3 mb-4">
          ...
          <div className="grid grid-cols-4 gap-3 lg:grid-cols-8 mb-5">
            <KPICard icon="🕐" label="Orders Waiting" value={pendingCount}
              color="bg-amber-50 border-amber-300 text-amber-800"
              active={activeKpiFilter==='pending'}
              onClick={() => setActiveKpiFilter(f=>f==='pending'?null:'pending')}
              sub="Not yet planned" />
            <KPICard icon="📋" label="Ready To Plan" value={draftCount}
              color="bg-blue-50 border-blue-300 text-blue-800"
              active={activeKpiFilter==='draft'}
              onClick={() => setActiveKpiFilter(f=>f==='draft'?null:'draft')}
              sub="Draft plans" />
            <KPICard icon="✅" label="Ready To Produce" value={(dash.byStatus||[]).find(b=>b.status==='PLANNED')?.count||0}
              color="bg-teal-50 border-teal-300 text-teal-800"
              active={activeKpiFilter==='ready'}
              onClick={() => setActiveKpiFilter(f=>f==='ready'?null:'ready')}
              sub="Confirmed" />
            <KPICard icon="🔴" label="RM Shortages" value={plans.filter(p=>p.rmCheckStatus==='RED'&&p.status!=='CANCELLED').length}
              color="bg-red-50 border-red-300 text-red-700"
              active={activeKpiFilter==='rmshort'}
              onClick={() => setActiveKpiFilter(f=>f==='rmshort'?null:'rmshort')}
              sub="Need procurement" />
            <KPICard icon="🦠" label="Biomass Shortages" value={0}
              color="bg-purple-50 border-purple-300 text-purple-800"
              active={false} onClick={() => {}}
              sub="Cold room check" />
            <KPICard icon="⚙" label="Running Today" value={inProgressCnt}
              color="bg-orange-50 border-orange-300 text-orange-800"
              active={activeKpiFilter==='running'}
              onClick={() => setActiveKpiFilter(f=>f==='running'?null:'running')}
              sub="In production" />
            <KPICard icon="🧪" label="Waiting For QC" value={plans.filter(p=>p.status==='AWAITING_QC').length}
              color="bg-yellow-50 border-yellow-300 text-yellow-800"
              active={activeKpiFilter==='qc'}
              onClick={() => setActiveKpiFilter(f=>f==='qc'?null:'qc')}
              sub="QC pending" />
            <KPICard icon="✔" label="Completed" value={completedCnt}
              color="bg-green-50 border-green-300 text-green-800"
              active={activeKpiFilter==='completed'}
              onClick={() => setActiveKpiFilter(f=>f==='completed'?null:'completed')}
              sub="This period" />
          </div>

          {/* Today's date + plant load */}
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              {pendingOrders.length > 0 && (
                <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2.5 py-0.5 rounded-full">{pendingOrders.length} waiting</span>
              )}
            </div>
            <button onClick={runEngine} disabled={running}
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            SECTION 3 — PLANNED PRODUCTION TASKS
        ═══════════════════════════════════════════════════════════════════ */}
        <section>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
            ...
            {activeKpiFilter && (
              <button onClick={() => setActiveKpiFilter(null)}
                className="text-xs text-indigo-600 border border-indigo-200 px-3 py-1.5 rounded-lg font-semibold hover:bg-indigo-50">
                ✕ Clear Filter
              </button>
            )}
          </div>
      ...
      {drawer && (
        <PlanningDrawer
          plan={drawer.plan}
          pendingItem={drawer.pendingItem}
          onSave={handleSave}
