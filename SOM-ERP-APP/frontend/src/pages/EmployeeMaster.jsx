...
function EmployeeForm({ initial, onSave, onCancel }) {
  const [form, setForm] = useState({
    name: '', email: '', phone: '', role: 'PRODUCTION', section: '', ...initial
  })
...
function PermissionsTab({ pages, defaults }) {
  ...
  const [perms, setPerms]               = useState({})  // role → Set of pagePaths
  const [saving, setSaving]             = useState(false)
  ...
  async function resetToDefault() {
    const defaultPaths = defaults[selectedRole] || []
    setPerms(p => ({ ...p, [selectedRole]: new Set(defaultPaths) }))
  }
        ...
        {ROLES.map(r => (
          <button key={r} onClick={() => setSelectedRole(r)}
            className={`px-3 py-1.5 rounded-full text-sm font-semibold transition ${
              selectedRole === r ? ROLE_COLORS[r] + ' ring-2 ring-offset-1 ring-current' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}>
            {r}
          </button>
        ))}
              ...
              <div className="px-4 py-2 flex flex-wrap gap-3">
                {groupPages.map(p => (
                  <label key={p.path} className="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" checked={current.has(p.path)} onChange={() => toggle(p.path)}
...
function CompanyTab() {
  ...
  const load = useCallback(async () => {
    const res = await employeeApi.listCompanies()
  ...
  async function add(e) {
    ...
    setSaving(true)
    try {
      await employeeApi.addCompany({ code, name })
      ...
      <form onSubmit={add} className="flex gap-3 items-end">
        <div>
          ...
          <input value={code} onChange={e => setCode(e.target.value.toUpperCase())}
            placeholder="e.g. AL-PTE" maxLength={12}
        ...
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)}
            placeholder="e.g. Agrilife PTE Ltd"
...
export default function EmployeeMaster() {
  ...
  const [tab, setTab]             = useState('employees')   // employees | permissions | companies
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [empRes, pageRes, defRes] = await Promise.all([
        employeeApi.list(),
        employeeApi.listPages(),
        employeeApi.roleDefaults(),
      ])
                  ...
                  {filtered.filter(e => e.isActive).map(emp => (
                    <tr key={emp.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <Badge text={emp.role} color={ROLE_COLORS[emp.role] || 'bg-gray-100 text-gray-600'} />
                      </td>
      ...
      {tab === 'permissions' && (
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <p className="text-sm text-gray-500 mb-5">
            Check the pages each role is allowed to see. Changes apply to all employees with that role.
          </p>
