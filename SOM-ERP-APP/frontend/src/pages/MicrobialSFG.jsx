 ...
 * Other plants: placeholder (coming soon)
 */
...
function MicrobialTab() {
  ...
  // Group containers by microbe name → type
  const grouped = {}
  ...
  // Map inward batches by container_id for fast lookup
  ...
  const keys = Object.keys(grouped).filter(k =>
    !searchMicrobe || k.toLowerCase().includes(searchMicrobe.toLowerCase())
  )
            ...
            >
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 700, color: Object.keys(types).length > 0 ? '#166534' : '#dc2626', background: '#dcfce7', borderRadius: '6px', padding: '2px 10px' }}>
                      ...
                      {typeData.containers.map(cont => {
                        ...
                        return (
                          <div key={cont.container_id} style={{ borderTop: '1px solid #f1f5f9' }}>
                            ...
                            {/* Batch rows (FIFO = oldest first) */}
                            {batches.length > 0 ? (
                              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                                <thead>
                                  <tr style={{ background: '#f1f5f9' }}>
                                    <th style={{ ...S.th, fontWeight: 600, padding: '5px 14px 5px 28px' }}>FIFO</th>
                                    <th style={{ ...S.th, padding: '5px 12px' }}>Batch Code</th>
                                    <th style={{ ...S.th, padding: '5px 12px' }}>Harvest Date</th>
                                    <th style={{ ...S.th, padding: '5px 12px' }}>Inhouse CFU/g</th>
                                    <th style={{ ...S.th, padding: '5px 12px' }}>Remaining (kg)</th>
                                    <th style={{ ...S.th, padding: '5px 12px' }}>Moisture</th>
                                    <th style={{ ...S.th, padding: '5px 12px' }}>Shelf Life</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {batches.map((b, fi) => (
                                    <tr key={b.inward_id} style={{ background: fi === 0 ? '#fffbeb' : 'transparent' }}>
                                      <td style={{ ...S.td, padding: '6px 12px 6px 28px' }}>
                                        {fi === 0 ? <span style={{ fontSize: '10px', fontWeight: 700, color: '#d97706', background: '#fef3c7', borderRadius: '4px', padding: '1px 6px' }}>NEXT</span>
