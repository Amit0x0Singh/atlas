/**
 * MicrobesMaster — Microbe Name & Code Master
 * Allows adding, editing, deleting, and bulk importing microbes via Excel.
 */
        ...
        <div style={S.card}>
            ...
            <>
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px', marginBottom: '16px' }}>
                ...
                <table style={{ ...S.table, fontSize: '12px' }}>
                  <thead>
                    <tr>
                      {Object.keys(importRows[0]).map(k => <th key={k} style={{ ...S.th, padding: '6px 10px' }}>{k}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {importRows.slice(0, 8).map((r, i) => (
                      <tr key={i}>
                        {Object.values(r).map((v, j) => <td key={j} style={{ ...S.td, padding: '6px 10px' }}>{String(v)}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {importStatus && (
            <div style={{ marginTop: '16px', fontSize: '13px' }}>
              {/* Summary banner — green if any imported, amber if all skipped */}
              <div style={{
              ...
              {/* Column mismatch hint — shown when all skipped and first error mentions columns */}
              {importStatus.imported === 0 && importStatus.skipped > 0 && (
                <div style={{ padding: '12px 16px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '8px', marginBottom: '10px' }}>
                  ...
                  <ul style={{ margin: 0, paddingLeft: '18px', color: '#92400e', lineHeight: '1.8' }}>
                    <li>Column headers in your Excel must be exactly <strong>Microbe Name</strong> and <strong>Microbe Code</strong> (case-sensitive)</li>
                    <li>Download the template below to see the exact format</li>
                    <li>Detected columns in your file: <strong>{importRows[0] ? Object.keys(importRows[0]).join(', ') : '—'}</strong></li>
                  </ul>
                </div>
              )}
