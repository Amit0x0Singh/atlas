...
export async function createDecanting(req, res) {
  ...
  if (!pack[0]) return res.status(404).json({ success: false, error: "Source pack not found" });
  if (!pack[0].qr_confirmed) return res.status(400).json({ success: false, error: "Source pack not QR confirmed" });
  if (Number(pack[0].qty_remaining) < Number(qty_to_transfer)) {
    return res.status(400).json({ success: false, error: `Insufficient qty. Available: ${pack[0].qty_remaining}` });
  }

  const container = await prisma.$queryRaw`SELECT * FROM erp_containers WHERE container_id = ${target_container_id}`;
  if (!container[0]) return res.status(404).json({ success: false, error: "Target container not found" });
  if (container[0].item_code !== pack[0].item_code) {
    return res.status(400).json({ success: false, error: `Container is for ${container[0].item_code}, pack is ${pack[0].item_code}. Item mismatch.` });
  }
  ...
  if (needsApproval && !req.body.supervisor_approved) {
    return res.status(422).json({
      success: false, needs_approval: true, variance_pct: variancePct.toFixed(3), tolerance_pct: tolerancePct,
  ...
  if (newContainerQty > Number(container[0].max_capacity)) {
    return res.status(400).json({ success: false, error: `Container overflow. Max capacity: ${container[0].max_capacity}, current: ${container[0].current_qty}, adding: ${actualQty}` });
  }
  ...
  if (newContainerQty <= Number(container[0].low_stock_threshold)) {
    await createNotification({
      ...
      targetRole: "store_manager", refType: "erp_container", refId: target_container_id,
    });
  }

  await writeAudit({ ...auditUser(req), action: "DECANT", tableName: "decanting_log", recordId: log[0].id, newValue: req.body });
  return res.status(201).json({ success: true, data: log[0] });
}
