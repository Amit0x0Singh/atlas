...
export async function createWarehouseTransfer(req, reply) {
  const {
    pack_id,
    transfer_type,
    from_location,
  ...
  if (!pack_id || !transfer_type) {
    return reply
      .status(400)
      .send({ success: false, error: "pack_id and transfer_type required" });
  }
  ...
  if (!pack[0].qr_confirmed) {
    return reply
      ...
      .send({
        success: false,
        error: "Pack must be QR-confirmed before transfer",
      });
  }
  ...
  if (transfer_type === "intra_plant") {
    ...
    return reply
      ...
      .send({
        success: true,
        data: rows[0],
        message: "Intra-plant transfer completed",
      });
  }
  ...
  return reply
    ...
    .send({
      ...
      data: rows[0],
      message:
        "Inter-plant transfer initiated. Pending receipt at destination.",
    });
}

export async function receiveWarehouseTransfer(req, reply) {
  const { to_location } = req.body || {};
  const transfer =
    await prisma.$queryRaw`SELECT * FROM warehouse_transfers WHERE transfer_id = ${req.params.id}::uuid`;
  if (!transfer[0])
    return reply
      .status(404)
      .send({ success: false, error: "Transfer not found" });
  if (transfer[0].status !== "pending")
    return reply
      .status(400)
      .send({ success: false, error: "Transfer is not pending" });
  ...
  return {
    success: true,
    message: "Transfer received and pack is now active at new location",
  };
}

export async function listWarehouseTransfers(req) {
