/**
 * Microbial SFG — Master Routes  /api/microbial-sfg/masters/*
 *
 *  GET    /microbes               — list all microbes
 *  POST   /microbes               — create one
 *  PUT    /microbes/:id           — update
 *  DELETE /microbes/:id           — delete
 *  POST   /microbes/import        — bulk import [{microbe_name, microbe_code}]
 *  POST   /migrate                — create tables if they don't exist (safe to call repeatedly)
 */
import prisma from '../db.js'

// ── Auto-create all Microbial SFG tables on plugin load ───────────────────────
async function ensureTables() {
  try {
    ...
    `)
    // Extend recipe_db with microbe columns (safe — IF NOT EXISTS)
...
export default async function microbialSfgMasterRoutes(fastify) {

  // Run table setup when plugin loads (safe to run multiple times)
  await ensureTables()

  // ── GET /migrate — manual trigger (useful for testing) ───────────────────────
  fastify.post('/migrate', async () => {
    await ensureTables()
    return { success: true, message: 'Microbial SFG tables ensured' }
  })

  // ── List all microbes ────────────────────────────────────────────────────────
  fastify.get('/microbes', async () => {
    const rows = await prisma.$queryRaw`
      SELECT * FROM microbe_master ORDER BY microbe_name ASC
    `
    return { success: true, data: rows }
  })

  // ── Get one microbe ──────────────────────────────────────────────────────────
  fastify.get('/microbes/:id', async (req, reply) => {
    const rows = await prisma.$queryRaw`
      SELECT * FROM microbe_master WHERE microbe_id = ${req.params.id}
    `
    if (!rows[0]) return reply.status(404).send({ success: false, error: 'Microbe not found' })
    return { success: true, data: rows[0] }
  })

  // ── Create microbe ───────────────────────────────────────────────────────────
  fastify.post('/microbes', async (req, reply) => {
    const { microbe_name, microbe_code } = req.body || {}
    if (!microbe_name || !microbe_code)
      return reply.status(400).send({ success: false, error: 'microbe_name and microbe_code are required' })

    const upper = microbe_code.trim().toUpperCase()
    try {
      const rows = await prisma.$queryRaw`
        INSERT INTO microbe_master (microbe_name, microbe_code)
        VALUES (${microbe_name.trim()}, ${upper})
        RETURNING *
      `
      return reply.status(201).send({ success: true, data: rows[0] })
    } catch (e) {
      if (e.message?.includes('unique') || e.message?.includes('duplicate')) {
        return reply.status(409).send({ success: false, error: `Microbe code "${upper}" already exists` })
      }
      throw e
    }
  })
  ...
  // ── Update microbe ───────────────────────────────────────────────────────────
  fastify.put('/microbes/:id', async (req, reply) => {
    const { microbe_name, microbe_code } = req.body || {}
    const rows = await prisma.$queryRaw`
  ...
  // ── Delete microbe ───────────────────────────────────────────────────────────
  fastify.delete('/microbes/:id', async (req, reply) => {
    await prisma.$executeRaw`
      DELETE FROM microbe_master WHERE microbe_id = ${req.params.id}
    `
    return { success: true }
  })

  // ── Bulk import microbes ─────────────────────────────────────────────────────
  // Body: { rows: [{ microbe_name, microbe_code }] }
  // Accepts any column naming: "Microbe Name", "microbe_name", "Name", "MICROBE NAME" etc.
  fastify.post('/microbes/import', async (req, reply) => {
    const { rows } = req.body || {}
    ...
    for (const r of rows) {
      // Flexible column matching — try all common variants
      ...
      try {
        await prisma.$executeRaw`
          INSERT INTO microbe_master (microbe_name, microbe_code)
          VALUES (${name}, ${code})
          ON CONFLICT (microbe_code) DO UPDATE
            SET microbe_name = EXCLUDED.microbe_name, updated_at = NOW()
        `
        imported++
      } catch (e) {
