...
export default async function salesOrderRoutes(fastify) {
  ...
  fastify.post('/', async (req, reply) => {
    ...
    if (!company || !diNo || !customerName || !orderType || !estimatedDispatchDate)
      return reply.status(400).send({ success: false, error: 'company, diNo, customerName, orderType, estimatedDispatchDate are required' })
