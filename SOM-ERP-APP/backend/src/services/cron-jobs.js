...
import { checkEscalations, createNotification } from './notification-service.js'
import { runPlanningEngine } from '../routes/erp-plan-engine.js'
...
function scheduleDailyAt(hour, minute, fn, log, label) {
  ...
  log.info && log.info(`[CRON ${label}] scheduled — next run in ${Math.round(msUntilNext() / MIN)} min`)
}
...
export function startCronJobs(fastify) {
  ...
  setInterval(async () => {
    try { await checkEscalations() }
    catch (e) { log.error('[CRON escalation]', e.message) }
  }, 15 * MIN)
  ...
  setInterval(async () => {
    try {
      ...
      if (pending.length) log.info(`[CRON qr_pending] Flagged ${pending.length} entries`)
    } catch (e) { log.error('[CRON qr_pending]', e.message) }
  }, 30 * MIN)

  // ── 3. CFU status updater: every 6 hours ──────────────────────────────────
  setInterval(async () => {
    try { await updateCfuStatuses(log) }
    catch (e) { log.error('[CRON cfu]', e.message) }
  }, 6 * HR)
  ...
  setInterval(async () => {
    try {
      ...
      if (unconfirmed.length) log.info(`[CRON microbial_unconfirmed] ${unconfirmed.length} dispatches flagged`)
    } catch (e) { log.error('[CRON microbial_unconfirmed]', e.message) }
  }, HR)
