...
export const DEFAULT_SALES_HISTORY_SORT = { field: 'etd', direction: 'asc' }
...
/** Centered popup — same pattern as InwardHistorySortModal / PackTableSortModal. */
export default function SalesHistorySortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="ETD (Est. Dispatch)" field="etd" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Soonest first' }, { value: 'desc', label: 'Latest first' }]} />
        <SortGroup title="Order Date" field="orderDate" active={value} onPick={onApply}
          options={[{ value: 'desc', label: 'Newest first' }, { value: 'asc', label: 'Oldest first' }]} />
        <SortGroup title="Customer" field="customer" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Total Qty" field="totalQty" active={value} onPick={onApply}
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_SALES_HISTORY_SORT)}>Reset</Button>
