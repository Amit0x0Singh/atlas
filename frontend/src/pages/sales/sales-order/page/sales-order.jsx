...
const SalesOrder = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      // Orders are critical — failure surfaces as an error
    ...
    // Products and profiles are non-critical — failures are silent
    try {
      const prodRes = await productApi.list({});
      setProducts(prodRes.data || []);
    } catch {
      /* product master unavailable — form picker will be empty */
    }
