...
export default function OutwardTab({ adjustView = null, outwardView = null, registerBackHandler }) {
  ...
  const selectTask = async (task) => {
    ...
    try {
      ...
      const microbeRows = res.microbes.map((m) => newRow(m))
      setRows(microbeRows)
      ...
      // Kick off FEFO for every row up front so allocations show without the
      // operator opening each one (calculateRow no-ops a covered row).
      microbeRows.filter((r) => Number(r.required_cfu_per_g) > 0).forEach((r) => calculateRow(r, { silent: true }))
