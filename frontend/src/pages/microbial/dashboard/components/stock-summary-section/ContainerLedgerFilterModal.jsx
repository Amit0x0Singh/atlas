...
export const EMPTY_CONTAINER_LEDGER_FILTERS = { typeFilter: '', statusFilter: '' }

/** Centered popup — same pattern as the Stock Ledger's LedgerFilterModal. */
export default function ContainerLedgerFilterModal({ open, onClose, value, onApply, types, statuses }) {
  ...
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            {value.statusFilter && <button type="button" onClick={() => set({ statusFilter: '' })} className="text-[11px] font-semibold text-blue-600 hover:underline">Reset</button>}
          </div>
          <select value={value.statusFilter} onChange={(e) => set({ statusFilter: e.target.value })} className={`${FIELD} cursor-pointer`}>
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(EMPTY_CONTAINER_LEDGER_FILTERS)}>Reset all</Button>
