...
export const DEFAULT_LEDGER_SORT = { field: 'timestamp', direction: 'desc' }
...
/** Centered popup — same pattern as RmMasterSortModal / ProductSortModal.
 *  The backend hardcodes `orderBy: { timestamp: 'desc' }` with no dynamic
 *  sort param (see ledger.controller.js), so this only re-sorts whichever
 *  page of rows is already loaded — same "only sorts what's loaded"
 *  compromise Product/Equipment Master use for their client-side sort. */
export default function LedgerSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Date & Time" field="timestamp" active={value} onPick={onApply}
          options={[{ value: 'desc', label: 'Newest first' }, { value: 'asc', label: 'Oldest first' }]} />
        <SortGroup title="Item Name" field="itemName" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Qty" field="qty" active={value} onPick={onApply}
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_LEDGER_SORT)}>Reset</Button>
