...
export const DEFAULT_INWARD_HISTORY_SORT = { field: 'receivedDate', direction: 'desc' }
...
/** Centered popup — same pattern as RmMasterSortModal / RmSortModal (stock page). */
export default function InwardHistorySortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Received Date" field="receivedDate" active={value} onPick={onApply}
        ...
        <SortGroup title="Supplier" field="supplier" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
      </div>

      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_INWARD_HISTORY_SORT)}>Reset</Button>
