...
export const DEFAULT_BOM_ISSUED_SORT = { field: 'date', direction: 'desc' }
...
export default function BomIssuedSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Last Issued" field="date" active={value} onPick={onApply}
          options={[{ value: 'desc', label: 'Newest first' }, { value: 'asc', label: 'Oldest first' }]} />
        <SortGroup title="Product" field="product" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A–Z' }, { value: 'desc', label: 'Z–A' }]} />
        <SortGroup title="Batch Qty" field="batchQty" active={value} onPick={onApply}
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_BOM_ISSUED_SORT)}>Reset</Button>
