...
export const EMPTY_BOM_ISSUED_FILTERS = { from: '', to: '' }
...
export default function BomIssuedFilterModal({ open, onClose, value, onApply }) {
  ...
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            {value.from && <button type="button" onClick={() => set({ from: '' })} className="text-[11px] font-semibold text-blue-600 hover:underline">Reset</button>}
          </div>
          <input type="date" value={value.from} onChange={e => set({ from: e.target.value })} className={FIELD} />
        </div>
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className={LABEL}>Issued up to</label>
            {value.to && <button type="button" onClick={() => set({ to: '' })} className="text-[11px] font-semibold text-blue-600 hover:underline">Reset</button>}
          </div>
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(EMPTY_BOM_ISSUED_FILTERS)}>Reset all</Button>
