...
export const DEFAULT_CONTAINER_SORT = { field: 'name', direction: 'asc' }
...
/** Centered popup, same interaction as RmSortModal on the Item Inventory page. */
export default function ContainerSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Item Name" field="name" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Container ID" field="containerId" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Current Qty" field="currentQty" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Ascending' }, { value: 'desc', label: 'Descending' }]} />
        <SortGroup title="Capacity" field="capacity" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Ascending' }, { value: 'desc', label: 'Descending' }]} />
        <SortGroup title="Fill %" field="fillPct" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Ascending' }, { value: 'desc', label: 'Descending' }]} />
      </div>

      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_CONTAINER_SORT)}>Reset</Button>
