...
export const EMPTY_CONTAINER_FILTERS = { status: '', uom: '', minQty: '', maxQty: '' }
...
/** Centered popup — same pattern as RmFilterModal on the Item Inventory page. */
export default function ContainerFilterModal({ open, onClose, value, onApply, uomOptions = [] }) {
  ...
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
        <div>
          ...
          <select value={value.status} onChange={e => set({ status: e.target.value })} className={`${FIELD} cursor-pointer`}>
            ...
            <option value="full">Full (&ge;75%)</option>
          </select>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className={LABEL}>Current Qty Range</label>
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(EMPTY_CONTAINER_FILTERS)}>Reset all</Button>
