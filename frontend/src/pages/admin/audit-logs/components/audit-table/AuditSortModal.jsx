...
// This table is server-paginated — sort is applied server-side via the
// `sortField`/`sortDir` query params the backend list endpoint honors,
// so switching sort re-fetches rather than reordering the loaded page.
export const DEFAULT_AUDIT_SORT = { field: 'createdAt', direction: 'desc' }
...
export default function AuditSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Date & Time" field="createdAt" active={value} onPick={onApply}
          options={[{ value: 'desc', label: 'Newest first' }, { value: 'asc', label: 'Oldest first' }]} />
        <SortGroup title="User" field="username" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Action" field="action" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
      </div>

      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_AUDIT_SORT)}>Reset</Button>
