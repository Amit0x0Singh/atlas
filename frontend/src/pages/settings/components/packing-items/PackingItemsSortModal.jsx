...
export const DEFAULT_PACKING_SORT = { field: 'name', direction: 'asc' }
...
const AZ = [{ value: 'asc', label: 'A–Z' }, { value: 'desc', label: 'Z–A' }]

/** Centered popup — same pattern as SupplierSortModal. */
export default function PackingItemsSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Name" field="name" active={value} onPick={onApply} options={AZ} />
        <SortGroup title="Item Code" field="itemCode" active={value} onPick={onApply} options={AZ} />
        <SortGroup title="Type" field="type" active={value} onPick={onApply} options={AZ} />
        <SortGroup title="Date Added" field="createdAt" active={value} onPick={onApply}
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_PACKING_SORT)}>Reset</Button>
