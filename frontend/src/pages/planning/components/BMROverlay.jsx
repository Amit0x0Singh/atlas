...
import { sfgAddEntry } from '../planningConstants.js'
...
function TickRow({ id, label, fieldKey, checkedDefault=true }) {
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 cursor-pointer hover:bg-blue-50 hover:border-blue-200 transition mb-1.5">
      <input type="checkbox" data-bmr-field={fieldKey} defaultChecked={checkedDefault} id={id}
...
function NanoBMR({ task, onSubmit, comps }) {
  ...
  return (
    <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <BInp type="time" data-bmr-field="tank-status-time" placeholder="Time" className="w-28"/>
                </div>
              </div>
            ...
            <BGrid>
              <div>
                ...
                <div className="flex gap-4">
                  {['Good','Unwell'].map(v => (
                    <label key={v} className="flex items-center gap-1.5 text-[13px] cursor-pointer">
                      <input type="radio" name={`eng-h-${task.id}`} data-bmr-field="eng-health" value={v} defaultChecked={v==='Good'}/> {v==='Good'?'😊':''} {v}
                    </label>
                  ))}
                </div>
                <div id={`eng-unwell-${task.id}`} className="hidden mt-2">
                  <BInp type="text" data-bmr-field="eng-unwell-detail" placeholder="Describe symptoms..." className="w-full border-red-200 bg-red-50"/>
                </div>
              </div>
              <div>
                <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Plant Workers / Helpers</div>
                <div className="flex gap-4">
                  {['Good','Unwell'].map(v => (
                    <label key={v} className="flex items-center gap-1.5 text-[13px] cursor-pointer">
          ...
          <BSection title="📋 Documents & PPE Check">
            <BGrid>
              <div>
                ...
                <div className="mt-2">
                  <BField label="RM Remarks"><BInp type="text" data-bmr-field="rm-remarks" placeholder="Any remarks on raw materials..." className="w-full"/></BField>
                </div>
              </div>
              <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <TickRow id={`maint-${task.id}`} label="Reactor Checked by Maintenance Team" fieldKey="maint-check" />
              <BField label="Tools Used"><BInp type="text" data-bmr-field="tools-used" placeholder="List tools used..." className="w-full"/></BField>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3">
      ...
      {/* Page 2: Reactor Cleaning */}
      {page === 1 && (
        <>
          <BSection title="🧹 Reactor Cleaning">
            <BGrid>
              <BField label="Reactor Cleaning Done"><BSel data-bmr-field="reactor-cleaning"><option>YES</option><option>NO</option></BSel></BField>
              ...
              <BField label="Maintenance Incharge Name"><BInp type="text" data-bmr-field="maint-incharge"/></BField>
            </BGrid>
          ...
          <BSection title="📦 Raw Material Received">
            ...
            <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-[12px] text-blue-700">
              📎 Attach the RM Sheet to this record
            </div>
          </BSection>
        </>
      )}

      {/* Page 3: Batch Process */}
      {page === 2 && (
        <>
          <BSection title="🚀 Batch Start">
            <BGrid>
              <BField label="Batch Start Time"><BInp type="time" data-bmr-field="batch-start-time"/></BField>
              <BField label="Batch Start Temperature (°C)"><BInp type="number" data-bmr-field="batch-start-temp" step="0.1"/></BField>
            </BGrid>
          </BSection>
          <BSection title="💧 DM Water Discharge">
            ...
            <div className="mt-3">
              <TickRow id={`wcc-${task.id}`} label="Checked for Water Contamination" fieldKey="water-contamination-check" checkedDefault={false} />
            </div>
          </BSection>
      ...
      {/* Page 4: Formulation Protocol */}
      {page === 3 && (
        <BSection title={`⚗️ Batch Formulation Protocol — ${comps.filter(c=>!c.isHeader).length} component(s)`}>
          ...
          <div className="mt-4 border-2 border-dashed border-amber-300 rounded-lg p-4 bg-amber-50">
            ...
            <textarea data-bmr-field="deviation-record" rows={3}
              placeholder="If any deviation from standard process, supervisor records here..."
        ...
        <>
          <BSection title="🔒 Batch Close-Out">
            <BGrid>
              ...
              <BField label="Reactor Switch-Off Time"><BInp type="time" data-bmr-field="reactor-off-time"/></BField>
              ...
              <BField label="Batch Comparison Notes"><BInp type="text" data-bmr-field="batch-comparison" placeholder="Comparison with previous batches..."/></BField>
            </BGrid>
          </BSection>
    ...
    <div>
      ...
      {page === 0 && (
        <BSection title="📋 Batch Header">
          <BGrid cols={4}>
            <BField label="Date"><BInp type="date" data-bmr-field="date" defaultValue={todayVal}/></BField>
            <BField label="Equipment Allotted"><BInp type="text" data-bmr-field="equipment" defaultValue={task.equipment||''}/></BField>
            <BField label="Temperature (°C)"><BInp type="number" data-bmr-field="temperature" step="0.1" placeholder="e.g. 26"/></BField>
            <BField label="Humidity & Weather"><BInp type="text" data-bmr-field="humidity-weather" placeholder="Clear / Humid / Rainy" defaultValue="Clear"/></BField>
      ...
      {page === 1 && (
        <BSection title="🧪 Formulation">
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <BField label="SFG DI No. / DOF / Qty"><BInp type="text" data-bmr-field="sfg-details" placeholder="DI No. / Date / Qty used"/></BField>
      ...
      {page === 2 && (
        <BSection title="⏱ Process Details">
          <BGrid>
            <BField label="RM Charging Start Time"><BInp type="time" data-bmr-field="rm-charge-start"/></BField>
            <BField label="RM Charging End Time"><BInp type="time" data-bmr-field="rm-charge-end"/></BField>
            <BField label="Blending Start Time"><BInp type="time" data-bmr-field="blend-start"/></BField>
            <BField label="Blending End Time"><BInp type="time" data-bmr-field="blend-end"/></BField>
            <BField label="Unloading Start Time"><BInp type="time" data-bmr-field="unload-start"/></BField>
            <BField label="Unloading End Time"><BInp type="time" data-bmr-field="unload-end"/></BField>
            ...
            <BField label="Mesh Size Used"><BInp type="text" data-bmr-field="mesh-size"/></BField>
            <BField label="Sieving Start Time"><BInp type="time" data-bmr-field="sieve-start"/></BField>
            <BField label="Sieving End Time"><BInp type="time" data-bmr-field="sieve-end"/></BField>
            <BField label="Incharge Name"><BInp type="text" data-bmr-field="process-incharge" defaultValue={task.incharge||''}/></BField>
      ...
      {page === 3 && (
        <BSection title="📦 Packing Details">
          <div className="grid grid-cols-3 gap-3">
            ...
            <BField label="Qty Received for Packing (kg)"><BInp type="number" data-bmr-field="packing-recv-qty" step="0.001"/></BField>
            ...
            <BField label="Weight of Unit Packing"><BInp type="text" data-bmr-field="unit-pack-weight" defaultValue={task.unitPackQty||''}/></BField>
            ...
            <BField label="Total No. of Outer Packages"><BInp type="number" data-bmr-field="total-outer-packs" defaultValue={task.totalSecPacks||''}/></BField>
            <BField label="Labels"><BInp type="text" data-bmr-field="labels" defaultValue={task.labels||''}/></BField>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <BField label="Stretch Film Wrapping"><BSel data-bmr-field="stretch-film"><option value="NO">NO</option><option value="YES">YES</option></BSel></BField>
            ...
            <BField label="Qty Left Over After Packing (kg)"><BInp type="number" data-bmr-field="leftover-qty" step="0.001"/></BField>
            <BField label="Leftover Qty Stored At"><BInp type="text" data-bmr-field="leftover-location"/></BField>
          </div>
        </BSection>
      )}
      ...
      {page === 4 && (
        <BSection title="🧪 QC Sampling">
          <BGrid>
            ...
            <BField label="Sample Collected at Which Process"><BInp type="text" data-bmr-field="sample-at-process"/></BField>
            ...
            <BField label="Incharge Name"><BInp type="text" data-bmr-field="qc-incharge" defaultValue={task.incharge||''}/></BField>
          </BGrid>
        </BSection>
      )}
    </div>
...
function QCHandoffModal({ task, bmrData, sfgCreated, onClose }) {
  ...
  const finalQty = sfgCreated ? sfgCreated.qty : (parseFloat(bmrData['total-qty-packed']) || parseFloat(bmrData['total-qty']) || task.qty)
  ...
  function confirmSend() {
    ...
    qcQueue.push({
      id: genId(), taskId: task.id,
      productName: task.productName, batchCode: task.batchCode,
      diNo: task.diNo, plant: task.plant,
      qty: task.qty, qtyUom: task.qtyUom,
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-5 bg-black/50">
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden">
        <div className="px-6 py-4" style={{background:'linear-gradient(135deg,#0f6e56,#0a4f3e)',color:'#fff'}}>
          <div className="text-base font-bold">🧪 Send to QC</div>
          <div className="text-[12.5px] opacity-80 mt-0.5">Batch signed off — confirm details before sending to QC</div>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[['Product', task.productName],['Batch No', task.batchCode||'—'],['DI Number', task.diNo||'—'],['Plant / Section', task.plant],['Qty Completed', `${finalQty} ${task.qtyUom||'kg'}`],['Stored At', storedWhere]].map(([l,v]) => (
              <div key={l}>
          ...
          <div className="mb-4">
            ...
            <textarea rows={2} value={remarks} onChange={e => setRemarks(e.target.value)}
              placeholder="Any notes for the QC team..."
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => onClose('skip')} className="px-4 py-2 border border-gray-200 rounded-lg text-[13px] font-semibold hover:bg-gray-50 transition">Skip for now</button>
            <button onClick={confirmSend} className="px-5 py-2 rounded-lg text-[13px] font-bold text-white transition" style={{background:'#0f6e56'}}>✓ Send to QC</button>
          </div>
        </div>
      </div>
    </div>
...
export default function BMROverlay({ openTaskIds, onClose, onTasksChange }) {
  ...
  const [qcHandoff, setQcHandoff]   = useState(null) // { task, bmrData, sfgCreated }
  const panelRef = useRef(null)
  let debTimer = useRef(null)
  ...
  useEffect(() => { if (!activeId && openEntries.length) setActiveId(openEntries[0].taskId) }, [openTaskIds])

  function saveCurrentDraft() {
    if (!activeId || !panelRef.current) return
    const data = {}
    panelRef.current.querySelectorAll('[data-bmr-field]').forEach(el => {
      const key = el.dataset.bmrField
  ...
  function handleSubmit(taskId) {
    saveCurrentDraft()
    if (!confirm('Submit BMR for this task? This will mark the BMR as completed.')) return
    ...
    if (task.process === 'Formulation' && task.packAfter === 'NO') {
      const sfgQty = parseFloat(bmrData['weight-after-sieve']) || parseFloat(bmrData['weight-after-unload']) || task.qty
      const sfgLocation = prompt(
        `Formulation complete — no packing scheduled.\n\nEnter storage location for SFG stock:\n(${task.productName} | ${task.batchCode} | ${sfgQty} ${task.qtyUom||'kg'})`,
        task.location || ''
      )
      if (sfgLocation === null) { alert('SFG location is required. BMR not submitted.'); return }
      sfgAddEntry({ productName:task.productName, batchCode:task.batchCode, plant:task.plant, qty:sfgQty, qtyUom:task.qtyUom||'kg', location:sfgLocation.trim()||task.location||'—', sourceTaskId:task.id })
      sfgCreated = { qty: sfgQty, location: sfgLocation.trim()||task.location||'—' }
    }
    ...
    onTasksChange(remaining)
    if (remaining.length === 0 || activeId === taskId) {
      setActiveId(remaining[0] || null)
    }
  }

  function handleQcClose(action) {
    setQcHandoff(null)
    if (openTaskIds.filter(id => id !== qcHandoff?.task?.id).length === 0) onClose()
  }
  ...
  try {
    ...
    recipe = recipes.find(r => r.name && task?.productName && r.name.toLowerCase() === task.productName.toLowerCase())
  } catch {}
  const comps = recipe ? (recipe.components||[]).filter(c => c && c.component) : []
  const isNanoBot = task && (task.plant === 'Nano' || task.plant === 'Botanical')

  return (
    <>
      <div className="fixed inset-0 z-[300] flex flex-col justify-end" style={{background:'rgba(0,0,0,.5)'}}>
        {/* Tabs bar */}
        <div className="bg-[#1e293b] flex items-center overflow-x-auto flex-shrink-0 border-t-2 border-[#334155]" style={{minHeight:44}}>
          <div className="text-[11px] font-bold text-[#94a3b8] px-3 whitespace-nowrap tracking-wider">📋 OPEN BMRs</div>
          <div className="flex flex-1 overflow-x-auto">
            {openEntries.map(e => {
              const col = PLANT_COLOR[e.task.plant] || '#64748b'
              const isAct = e.taskId === activeId
              return (
                <div key={e.taskId}
                  className={`flex items-center gap-2 px-4 py-2.5 text-[12px] font-semibold cursor-pointer border-r border-[#334155] min-w-[160px] flex-shrink-0 transition-colors ${isAct ? 'bg-blue-700 text-white' : 'text-[#94a3b8] hover:bg-[#334155] hover:text-white'}`}
                  ...
                  <button className="opacity-60 hover:opacity-100 text-sm ml-1" onClick={ev => { ev.stopPropagation(); handleCloseTask(e.taskId) }}>✕</button>
                </div>
              )
            })}
          </div>
        ...
        {/* BMR Content */}
        {task && (
          <div className="bg-white flex flex-col" style={{height:'82vh',overflowY:'auto'}} ref={panelRef}
            onChange={() => { saveCurrentDraft(); showSavedStatus() }}>
            {/* Header */}
            <div className="sticky top-0 z-10 flex items-center justify-between px-7 py-4 text-white flex-shrink-0"
              ...
              <div>
                ...
                <div className="text-[12px] opacity-80 mt-0.5">
                  DI: {task.diNo||'—'} &nbsp;|&nbsp; Batch: {task.batchCode||'—'} &nbsp;|&nbsp;
                  Qty: {task.qty} {task.qtyUom||''} &nbsp;|&nbsp; Incharge: {task.incharge||'—'}
                  {task.carrier ? ` | Carrier: ${task.carrier}` : ''}
                  {task.specs   ? ` | Specs: ${task.specs}` : ''}
                </div>
              </div>
            ...
            {/* BMR pages */}
            {isNanoBot
              ? <NanoBMR task={task} comps={comps} onSubmit={handleSubmit}/>
              : <PowderBMR task={task} comps={comps}/>
            }
