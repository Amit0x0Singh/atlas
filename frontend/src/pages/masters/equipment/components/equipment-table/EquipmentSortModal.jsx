...
// Sorts only the rows currently loaded on screen (this table's data is
// server-paginated — see EquipmentMaster.jsx) rather than the full filtered
// set across every page.
export const DEFAULT_EQUIPMENT_SORT = { field: 'name', direction: 'asc' }
...
export default function EquipmentSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Equipment Name" field="name" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Equipment Code" field="equipCode" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Working Volume" field="workingVolume" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Ascending' }, { value: 'desc', label: 'Descending' }]} />
      </div>

      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_EQUIPMENT_SORT)}>Reset</Button>
