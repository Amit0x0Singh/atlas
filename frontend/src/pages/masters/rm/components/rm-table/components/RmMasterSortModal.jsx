...
export const DEFAULT_RM_MASTER_SORT = { field: 'name', direction: 'asc' }
...
/** Centered popup — same pattern as UsersSortModal / RmSortModal (stock page). */
export default function RmMasterSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        ...
        <SortGroup title="Item Code" field="itemCode" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Re-order Level" field="lowStockLevel" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Ascending' }, { value: 'desc', label: 'Descending' }]} />
      </div>

      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_RM_MASTER_SORT)}>Reset</Button>
