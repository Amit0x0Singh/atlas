...
export const DEFAULT_USER_SORT = { field: 'signupDate', direction: 'desc' }
...
/**
 * Centered popup (matches UsersFilterModal) — clicking any radio sets that
 * group as the active single sort key + direction; the highlighted radio
 * across the whole modal always reflects the one sort actually applied.
 */
export default function UsersSortModal({ open, onClose, value, onApply }) {
  return (
    <Modal open={open} onClose={onClose} size="sm">
      ...
      <div className="px-5 py-1 max-h-[70vh] overflow-y-auto">
        <SortGroup title="Sign-up Date" field="signupDate" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'Ascending' }, { value: 'desc', label: 'Descending' }]} />
        <SortGroup title="Name" field="name" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
        <SortGroup title="Role" field="role" active={value} onPick={onApply}
          options={[{ value: 'asc', label: 'A-Z' }, { value: 'desc', label: 'Z-A' }]} />
      </div>

      <div className="flex items-center justify-between gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50/60">
        <Button variant="outline-gray" onClick={() => onApply(DEFAULT_USER_SORT)}>Reset</Button>
